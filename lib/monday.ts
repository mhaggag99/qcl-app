...
async function gql(query: string, variables?: Record<string, unknown>, token?: string) {
  const tok = (token || process.env.MONDAY_API_TOKEN || "").trim();
...
async function findBoard(name: string, token?: string): Promise<{ id: string; name: string } | null> {
  const target = name.toLowerCase();
  let page = 1;
  while (true) {
    const data = await gql(`{ boards(limit: 500, page: ${page}) { id name } }`, undefined, token);
...
export async function findClientBoard(clientName: string, token?: string): Promise<string | null> {
  const data = await gql("{ boards(limit: 500) { id name } }", undefined, token);
  const boards: { id: string; name: string }[] = data?.data?.boards || [];
...
export async function findBoardItem(boardId: string, itemName: string, token?: string): Promise<string | null> {
...
export async function postUpdate(itemId: string, body: string, token?: string): Promise<boolean> {
  const data = await gql(
...
export async function postToClientBoard(
  ...
  const boardId = await findClientBoard(clientName, token);
  if (!boardId) return { ok: false, bubble, error: `No Monday board found for "${clientName}"` };

  const itemId = await findBoardItem(boardId, bubble, token);
  if (!itemId) return { ok: false, bubble, error: `Bubble "${bubble}" not found on board` };

  const ok = await postUpdate(itemId, noteText, token);
  return { ok, bubble };
}
...
export async function postToMCL(
  ...
  const board = await findBoard("master client list", token);
  ...
  const groupsData = await gql(`{ boards(ids: [${board.id}]) { groups { id title } } }`, undefined, token);
  ...
  for (const group of groups) {
    ...
    do {
      ...
      if (item) {
        const bubble = classifyBubble(noteText);
        const ok = await postUpdate(item.id, noteText, token);
        return { ok, bubble };
      }
...
export async function getVAAttendance(token?: string): Promise<VAAttendanceEntry[]> {
  const BOARD_ID = "18403901977"; // QCL Team Attendance Tracker

  const colData = await gql(`{ boards(ids: [${BOARD_ID}]) { columns { id title } } }`, undefined, token);
...
export async function getRoundtableStatus(token?: string): Promise<{ boardName: string; events: RoundtableEvent[] } | null> {
  const board = await findBoard("Client Roundtable Status", token);
  if (!board) return null;

  const colData = await gql(`{ boards(ids: [${board.id}]) { columns { id title } } }`, undefined, token);
  const columns: { id: string; title: string }[] = colData?.data?.boards?.[0]?.columns || [];
  const colId = (title: string) => columns.find((c) => c.title === title)?.id || "";
...
async function getActivityBoardId(token?: string): Promise<{ boardId: string; boardName: string } | null> {
  const now = new Date();
  const monthName = now.toLocaleDateString("en-US", { month: "long" });
  const year = now.getFullYear();
  const target = `${monthName} ${year}: Client Activity Tracking`;
  const board = await findBoard(target, token);
...
export async function getMonthlyActivity(token?: string): Promise<{ boardName: string; rows: ActivityRow[] } | null> {
  const found = await getActivityBoardId(token);
  if (!found) return null;
  const { boardId, boardName } = found;

  // Step 1: get columns to build title→id map
  const colData = await gql(`{ boards(ids: [${boardId}]) { columns { id title } } }`, undefined, token);
  const columns: { id: string; title: string }[] = colData?.data?.boards?.[0]?.columns || [];
  const colId = (title: string) => columns.find((c) => c.title === title)?.id || "";
